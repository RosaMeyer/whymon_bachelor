...
(* Proof module, defining types for a logical proof system with temporal operators, 
predicate logic, and more advanced features like handling regular expressions *)
  ...
  and sr_at = function
    ...
    | SConcat (rsp1, rsp2) -> let (tp1, _) = sr_at rsp1 in
                              let (_, tp4) = sr_at rsp2 in
                              (tp1, tp4) 
    | SStarEps tp -> (tp, tp)
    | SStar rsps -> let (tp1, _) = sr_at (Fdeque.peek_front_exn rsps) in
  ...
  module Size = struct
    ...
    and v = function
      ...
      | VPrexOut _ -> 1
      | VPrex (_, vps) -> 1 + sum vr vps
      | VFrex (_, vps) -> 1 + sum vr vps
    ...
    and vr = function
      ...
      | VConcat vrps -> 1 + sum vr vrps (* List of regex proofs *)
